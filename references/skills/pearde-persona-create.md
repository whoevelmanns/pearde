...
Read @references/personas/INDEX.md — the section **`persona create <topic>`**
is the procedure in order, six steps, and the section under it is the file
format. The scope is `@@personas`.

Two things that section turns on, so that nothing else has to be read first:
**a persona is built from research, never invented**, and **it is not live
until it is registered everywhere step 6 names** — the roster row, the signals
table in @references/parts/personas.md, `@@personas` in @index.md, and its row
