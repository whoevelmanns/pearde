...
`bash <skill>/doctor.sh --fix` answers whether that worked, for all three steps
below plus the board itself, and repairs what it can. Run it after installing,
and whenever a part is silent when it should not be: an install that is present
and broken does nothing, exactly like one that is absent.
...
what makes `pearde` invocable as a command.
...
`references/system.md` to it, commonly `AGENTS.md` at the repo root; create it
if absent. Replace `<skill>` in the block with the skill folder's actual path.
It carries `pearde:begin`/`:end` markers: marker present means installed,
leave it alone.

Nothing else to fill in: the first run creates `prds/settings.md` and asks
the user for the board language, per `references/settings.md`.

## 2. Status line

Optional. `statusline.sh` renders `<dir> <branch> · <model>`, plus
`▸pearde <d>/<n> <p>% · open <o> <q>%` when a board is in scope. It walks up from
...
Wire it in the config that is **in force**. `$CLAUDE_CONFIG_DIR` moves the whole
config elsewhere, so a machine can hold several profiles, each with its own
`settings.json` and its own status line. An entry written into the wrong one is
correct and inert: the file loads for a different profile. `doctor.sh` prints
the config directory it read.

A symlink is the one that rots: the config still names a command, the command
still names a path, and the path resolves to nothing, so the line renders empty
and reads as "no status line configured". `doctor.sh` reports that as `broken`
and `--fix` repoints it.
...
Only the board segment is pearde's — drop the dir/branch/model part if the
existing line already shows it.

## 3. Plane

Optional. Mirrors the board as tickets in a self-hosted
[Plane](https://github.com/makeplane/plane), running inside the skill.
Requires Docker and Python 3, nothing else.
...
No login anywhere: `bootstrap` creates everything itself, and `start` pins the
app to `127.0.0.1` and injects auto-login, so the browser opens straight into
the workspace. The knobs — password login back, LAN exposure — are in
`plane/plane.md`, with the config keys, the manual path, and what maps to
what. Port taken? Set `PLANE_PORT=<n>` before the first `install`.

A running app is not a mirror. Each board needs its own `bootstrap`, which
writes that board's `prds/.plane.env`; `plane.sh status [board]` and
`doctor.sh` both report a board the app is up for and has never been
bootstrapped for.

`plane-app/` and the fetched `setup.sh` are machine-local and gitignored; a
fresh clone of the skill re-runs `install` on each machine. Data lives in
