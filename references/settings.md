...
| `name`        | inferred     | what the board calls itself — the Plane project, the local timeline's title. Inferred from the directory on a plain board and from the member names on a master (`mitosys+model`), which is a placeholder: the first round that meets an unnamed master board asks the user and writes the answer here |
